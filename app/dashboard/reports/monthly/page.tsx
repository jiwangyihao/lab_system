...
export default function MonthlyReportsPage() {
  ...
  const loadReports = async () => {
    setIsLoading(true);
    const result = await getReports("MONTHLY");
  ...
  const handleGenerate = () => {
    startTransition(async () => {
      ...
      if (result.success) {
        toast.success("生成成功", { description: "月报表已生成" });
        await loadReports();
        if (result.data?.id) {
  ...
  const getSelectedLabel = () => {
    ...
    return format(new Date(report.periodStart), "yyyy年MM月", { locale: zhCN });
  };
            ...
            <SelectContent>
              {reports.map((report) => (
                <SelectItem key={report.id} value={report.id}>
                  {format(new Date(report.periodStart), "yyyy年MM月", {
                    locale: zhCN,
                  })}
                </SelectItem>
              ))}
      ...
      {selectedReport ? (
        ...
        <Card>
          <CardContent className="flex flex-col items-center justify-center py-12">
            <p className="text-muted-foreground mb-4">暂无月报表</p>
            <Button onClick={handleGenerate} disabled={isPending}>
              <IconPlus className="h-4 w-4 mr-2" />
              生成第一份月报表
            </Button>
